...
func TestRelaxedFuzzy(t *testing.T) {
	...
	tests := []struct {
		...
		{"bubo", "Bbo bubo", "Bubo bubo", false, 1, vlib.FuzzyRelaxed},
		{"bubo", "Bubo bbo onetwo", "Bubo bubo", false, 1, vlib.PartialFuzzyRelaxed},
		{"bubo", "Bubo bubo onetwo", "Bubo bubo", false, 0, vlib.PartialExact},
		{"pomatomus", "Pomatomu", "Pomatomus", true, 1, vlib.FuzzyRelaxed},
		{"pomatomus part", "Pomatomu aadsdss", "Pomatomus", true, 1, vlib.PartialFuzzyRelaxed},
	}
	...
	for _, v := range tests {
		...
		if v.uniFuzzy {
			params.WithUninomialFuzzyMatch = true
		}
		enc := gnfmt.GNjson{}
		...
		match := res.Matches[0].MatchItems[0]
		assert.Equal(v.editDist, match.EditDistance, v.msg)
		assert.Equal(v.typ, match.MatchType, v.msg)
		assert.Equal(v.typ, matches[0].MatchType, v.msg)
	}
}
...
func TestDataSources(t *testing.T) {
	...
	tests := []struct {
		...
		{"no ds", "Narcissus minor minor", []int{}, []int{165, 167, 169}, 1, []string{"Exact"}},
